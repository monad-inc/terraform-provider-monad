...
~> The provider is pre-1.0. Breaking changes ship as **minor** version bumps, so pin to a minor series (`~> 0.5.0`, which accepts 0.5.x patches only) and treat each minor upgrade as a reviewed change. The [changelog](https://github.com/monad-inc/terraform-provider-monad/blob/main/CHANGELOG.md) documents a migration path for each one.
...
* `base_url` - (Optional) Base URL of the Monad platform, **without** the `/api` suffix — the provider appends it. Falls back to `MONAD_BASE_URL`, then to `https://app.monad.com`, the Monad SaaS platform. Set it to your own host for a self-hosted deployment.
...
The provider has no `version` argument for connectors. A component Terraform creates gets its connector type's latest version. A component whose version was pinned outside Terraform (in the Monad UI or through the API) keeps that version across Terraform updates, including after `terraform import`. A component that Terraform destroys and re-creates gets the latest version again.
